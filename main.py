...
#主函数
def main(user, passwd, step, push_type, push_key):
    user = str(user)
    password = str(passwd)
    step = str(step)
    if user == '' or password == '':
    ...
    #print(response)
    title='小米运动'
    url="https://fm.m.tencent.com/an:mhr/sports/"
    msg="小米运动 步数修改为{}步 {}".format(step, url)
    if push_type == "1":
        send_wechat(push_key, title, msg)
    if push_type == "2":
        send_wechat_igot(push_key, title, msg, url)
...
if __name__ ==  "__main__":
    ...
    want_step = ''
    try:
        push_type, push_key, user_name, passwd, want_step = sys.argv[1:6]
    except Exception as e:
        print(sys.argv)
        print("usage: python3 main.py push_type push_key user_name passwd want_step")
        exit(e)
    setp_array = want_step.split('-')
    if len(setp_array) == 2:
        want_step = str(random.randint(int(setp_array[0]),int(setp_array[1])))
    elif str(want_step) == '0':
        want_step = ''


    main(user_name, passwd, want_step, push_type, push_key)
